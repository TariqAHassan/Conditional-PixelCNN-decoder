...
class PixelCNN(object):
    def __init__(self, X, conf, h=None):
        ...
        for i in range(conf.layers):
            ...
            with tf.variable_scope("v_stack" + i):
                v_stack = GatedCNN([filter_size, filter_size, conf.f_map], v_stack_in,
                                   mask=mask, conditional=self.h).output()
                v_stack_in = v_stack
            ...
            with tf.variable_scope("h_stack" + i):
                h_stack = GatedCNN([1, filter_size, conf.f_map], h_stack_in, payload=v_stack_1, mask=mask,
                                   conditional=self.h).output()
        ...
        if conf.data == "mnist":
            ...
            # Since this code was not run on CIFAR-10, I'm not sure which would be
            # a suitable way to generate 3-channel images. Below are the 2 methods
            # which may be used, with the first one (self.pred) being more likely.
            ...
            self.pred = tf.reshape(tf.argmax(tf.nn.softmax(
                self.fc2), dimension=tf.rank(self.fc2) - 1), tf.shape(self.X))
